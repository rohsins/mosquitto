...
rc = 1
connect_packet = mqtt_packets.gen_connect("subpub-qos0-test", proto_ver=5, clean_session=False, session_expiry=60)
connack1_packet = mqtt_packets.gen_connack(rc=0, proto_ver=5)
connack2_packet = mqtt_packets.gen_connack(rc=0, proto_ver=5, flags=1)

mid = 53
subscribe_packet = mqtt_packets.gen_subscribe(mid, "subpub/qos1", 1, proto_ver=5)
suback_packet = mqtt_packets.gen_suback(mid, 1, proto_ver=5)



helper_connect = mqtt_packets.gen_connect("helper", proto_ver=5)
helper_connack = mqtt_packets.gen_connack(rc=0, proto_ver=5)

mid=1
props = mqtt5_props.gen_uint32_prop(mqtt5_props.MESSAGE_EXPIRY_INTERVAL, 5)
publish1s_packet = mqtt_packets.gen_publish("subpub/qos1", mid=mid, qos=1, payload="message1", proto_ver=5, properties=props)
puback1s_packet = mqtt_packets.gen_puback(mid)

mid=2
props = mqtt5_props.gen_uint32_prop(mqtt5_props.MESSAGE_EXPIRY_INTERVAL, 100)
publish2s_packet = mqtt_packets.gen_publish("subpub/qos1", mid=mid, qos=1, payload="message2", proto_ver=5, properties=props)
puback2s_packet = mqtt_packets.gen_puback(mid)

mid=3
publish3_packet = mqtt_packets.gen_publish("subpub/qos1", mid=mid, qos=1, payload="message3", proto_ver=5)
puback3_packet = mqtt_packets.gen_puback(mid)
...
try:
    ...
    for i in range(100, 1, -1):
        ...
        publish2r_packet = mqtt_packets.gen_publish("subpub/qos1", mid=2, qos=1, payload="message2", proto_ver=5, properties=props)
