...
int handle__accepted_publish(struct mosquitto *context, struct mosquitto__base_msg *base_msg, uint16_t mid, int dup, uint32_t *message_expiry_interval)
{
	...
	if ((db.config->serverContext != NULL) && strcmp((char *) context->id, (char *) db.config->server_client_id)) {
		...
		size_t cpacket_length = strlen(context->address) + strlen(context->id) + username_length + strlen(stored->data.topic) + 100;
		...
		send__real_publish(
			...
			stored->data.payloadlen, // payloadlength
			stored->data.payload, // payload
			0, // qos
			false, // retain
			false,  // dup
