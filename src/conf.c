...
{
	mosquitto_FREE(config->server_client_id);
	mosquitto_FREE(config->server_topic);
	mosquitto_FREE(config->clientid_prefixes);
	mosquitto_FREE(config->persistence_location);
	mosquitto_FREE(config->persistence_file);
		...
		if((*buf)[0] != '#' && (*buf)[0] != 10 && (*buf)[0] != 13){
			...
			if(token){
				if(!strcmp(token, "accept_protocol_versions")){
					...
					if(conf__parse_string(&token, "server_topic", &config->server_topic, &saveptr)) {
						return MOSQ_ERR_INVAL;
					}
				}else if (!strcmp(token, "server_client_id")){
					if(conf__parse_string(&token, "server_client_id", &config->server_client_id, &saveptr)) {
						return MOSQ_ERR_INVAL;
					}
